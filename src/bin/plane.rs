use ray_tracer::shapes::{Sphere, Shape, Plane};
...
fn main() {
    ...
    let floor = Rc::new(Plane::new().with_material(&floor_m).with_transform(&Matrix::translation(0.0, 0.0, 0.0)));
    ...
    let right = Rc::new(Sphere::new().with_material(&right_m).with_transform(&(Matrix::translation(1.5, 0.5, -0.5) * Matrix::scaling(0.5, 0.5, 0.5))));

    let mut left_m = Material::new();
    left_m.color = Color::new(0.3, 0.3, 0.35);
    left_m.diffuse = 0.7;
    left_m.specular = 0.3;
    let left = Rc::new(Sphere::new().with_material(&left_m).with_transform(&(Matrix::translation(-1.5, 0.33, -0.75) * Matrix::scaling(0.33, 0.33, 0.33))));
